...
本次研究特別關注以下幾個問題：
首先，哪些 unit 能夠集中抓取 phoneme 的特徵，不會與其他 phoneme 混淆？其次，如果一個 unit 分散地對應到多個 phoneme，那麼這些 phoneme 可能是哪幾個？

針對後一個問題，我們從圖中發現了一些特定的趨勢，這些 unit 的特徵如下圖 \ref{fig:enter-label} 所示：
...
從這些圖中，我們可以看到，互相混淆的 phoneme 多數屬於同一個 

}}

{{{  % chatgpt-2
對於一個單位編碼系統的評估，我們特別關注的是每個單位所代表的音素是否集中，以及每個音素是否可以由較少的單位來代表。理想情況下，當某個單位出現時，我們應該幾乎能夠確定它代表的是哪一個音素。

這兩個指標可以在圖上通過顏色深淺來直觀地展示：
首先，音素純度（phoneme purity）表示每個列（column）取最大值後相加的總和。其次，單位純度（unit purity）表示每個行（row）取最大值後的 pxy 總和。

從這裡我們可以看到，單位純度受到每個單位只能取 41 個 pxy 總和的限制。而當群集（cluster）數量增加時，音素純度可以取得更多的 pxy 總和。這也意味著當群集數量與框架（frame）數量相同時，音素純度可以達到 100\%。另一方面，當群集數量增加時，每個格子的 pxy 會因單位數量的增多而被稀釋，但單位純度的 pxy 總和受到音素數量的限制，因此當群集數量增加時，單位純度會降低，這一點可以從圖中觀察到。

參考前人的研究，由於單位是假設，我們主要關注的是每個單位如何對應到音位標註上。為了更直觀地觀察這一點，我們將熱圖呈現為 p(p|u)，即對每個值進行歸一化，以顯示每個單位對應到哪個音素，以及這種對應是集中還是分散。如下圖所示：
...
本次研究特別關注以下幾個問題：
首先，哪些單位能夠集中抓取音素的特徵，不會與其他音素混淆？其次，如果一個單位分散地對應到多個音素，那麼這些音素可能是哪幾個？

針對後一個問題，我們從圖中發現了一些特定的趨勢，這些單位的特徵如下圖所示：
...
從這些圖中，我們可以看到，互相混淆的音素多數屬於同一個音素類別，或是發音方式有某種共通特性。基於此觀察，我們有理由重新排列音素的順序，並按照音素類別將這些音素分組，從而在整張熱圖的橫軸與縱軸進行分組觀察。如下圖所示：

{
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{figures/groupedheatmap.png}
    \caption{重新分組後的熱圖}
    \label{fig:grouped-heatmap}
\end{figure}
}
}}}
