...
\documentclass[12pt, a4paper]{z__utils/functions/ntuthesis}
...
\usepackage[colorlinks, linkcolor=black, citecolor=blue, urlcolor=blue, unicode]{hyperref}
...
% 請用 Adobe Acrobat 加入
% 也會需要加入 doi 的浮水印
%\input{z__utils/functions/watermark}
%% <<< 浮水印

% 如需額外的頁楣 (header) 或 footer，請在 z__utils/functions/headerfooter.tex 裡依例修改
% 它的預設內容是都關掉，可依需要打開
\input{z__utils/functions/headerfooter}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% 常見的 notation 指令
% 請修改 z__utils/functions/z__utils/functions/notations.tex 加入自己需要的 notations
\input{z__utils/functions/notations}

% Title Page
% 請修改 z__utils/functions/title_settings.tex 設定論文題目/指導教授等變數
\input{z__utils/contents/TEMPtitle_settings}

% 載入中文名詞的定義：例如，Figure -->「圖」, Chapter -->「第 x 章」
\input{z__utils/functions/ntu_definitions}

% 如果不需要以中文數字一、二、三呈現章別，例如「第一章」
% 則請把下列介於 >>> 與 <<< 之間
% 的文字行關掉 (行首加上百分號), 會以「第 1 章」呈現
%% 中文數字章別 >>>
\input{z__utils/functions/ntu_chnum}
%% <<< 中文數字章別
